/**
 * BTMTravel COBT - Create Demo Users (Backend Version)
 * Place this file in your backend directory and run it
 * 
 * Usage:
 *   node backend-create-demo-users.js
 */
...
async function createDemoUsers() {
  ...
  try {
    ...
    // Generate SQL - trying different postgres users
    let sql = `-- BTMTravel COBT - Demo Users Insert Script
-- Generated on: ${new Date().toISOString()}
-- Password for all accounts: ${password}
-- Bcrypt hash: ${hash}

-- Try to connect to database (change user if needed)
\\c cobt_user;
...
`;
    ...
    console.log('\n   Option 1: Copy and run SQL file');
    console.log('   ─────────────────────────────────');
    console.log(`   docker cp ${filename} cobt-postgres:/tmp/${filename}`);
    console.log(`   docker exec -it cobt-postgres psql -U root -f /tmp/${filename}`);
    console.log('');
    console.log('   Option 2: If "root" user fails, try these usernames:');
    console.log('   ─────────────────────────────────────────────────────');
    console.log(`   docker exec -it cobt-postgres psql -U postgres -f /tmp/${filename}`);
    console.log(`   docker exec -it cobt-postgres psql -U admin -f /tmp/${filename}`);
    console.log(`   docker exec -it cobt-postgres psql -U cobt -f /tmp/${filename}`);
    console.log('');
    console.log('   Option 3: Run SQL directly (EASIEST)');
    console.log('   ─────────────────────────────────────────');
    console.log('   1. docker exec -it cobt-postgres bash');
    console.log('   2. psql -U root cobt_user  (or psql cobt_user if root fails)');
    console.log('   3. Paste the SQL commands from the file');
    console.log('\n✅ Done!\n');
